...
def parse_args():
    ...
    parser.add_argument("--data_path", type=str, default="./crackseg_test_data_demo", help="test data path")
    ...
    parser.add_argument("--sam_checkpoint", type=str, default='./workdir/models/sam/sam_l_1024_halfadap_pca_lds_bmt.pth', help="sam checkpoint")
...
def main(args):
    ...
    test_dataset = TestingDataset(data_path=args.data_path, image_size=args.image_size, mode='test', requires_name=True, point_num=args.point_num, return_ori_mask=True, prompt_path=args.prompt_path,input_mask=False)
    ...
    for i, batched_input in enumerate(test_pbar):
        ...
        if args.boxes_prompt:
            ...
            save_path = os.path.join(f"{args.work_dir}", args.run_name, f"iter{args.iter_point if args.iter_point > 1 else args.point_num}_prompt_figer")
