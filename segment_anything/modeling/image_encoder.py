...
class Adapter_Layer(nn.Module):
    ...
    def __init__(self, embed_dim, mlp_ratio=0.25, norm_layer = nn.LayerNorm, skip_connect=True, scale: float = 0.1):
        ...
        # bottleneck: down_projection -> GELU -> up_projection
        self.down_projection = nn.Linear(embed_dim, hidden_dim, bias=True)
        self.act = nn.GELU()
        self.up_projection = nn.Linear(hidden_dim, embed_dim, bias=True)

        # scaling factor for adapter output
        self.scale = scale

        # init: small weights for stable fine-tuning
    ...
    def forward(self, x):
        ...
        # bottleneck in token space
        x_flat = x  # (B, H, W, C)
        x_flat = self.down_projection(x_flat)
        x_flat = self.act(x_flat)
        x_flat = self.up_projection(x_flat)

        # scale adapter contribution
        x_flat = x_flat * self.scale
...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #-------------------
        embedding_feature = self.prompt_generator.init_embeddings(x) #--linear
        handcrafted_feature = self.prompt_generator.init_handcrafted(inp) #--fft
        prompt = self.prompt_generator.get_prompt(handcrafted_feature, embedding_feature) #lightmlp
        #-------------------
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        #------------------------------------------------------------------------------------------------------
        #self.Adapter = MixtralSparseMoeBlock(4,2,0.001,transformer_dim // 8)
        #------------------------------------------------------------------------------------------------------
        self.attn = Attention(
            dim,
            num_heads=num_heads,
        ...
        #==================================================
        if self.adapter and self.window_size == 0:
