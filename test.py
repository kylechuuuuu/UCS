...
def parse_args():
    ...
    parser.add_argument("--work_dir", type=str, default="results", help="work dir")
    parser.add_argument("--run_name", type=str, default="demo", help="run model name")
    parser.add_argument("--batch_size", type=int, default=1, help="batch size")
    parser.add_argument("--image_size", type=int, default=1024, help="image_size")
    parser.add_argument('--device', type=str, default='cuda')
    parser.add_argument("--data_path", type=str, default=r"/hy-tmp/UCS/TMM_datasets/wire", help="test data path")
    ...
    parser.add_argument("--sam_checkpoint", type=str, default=r'/hy-tmp/UCS/weight/models/sam/sam_l_1024_halfadap_pca_ldsbmt_5.pth', help="sam checkpoint")
...
def main(args):
    ...
    # 'occlusion', 'salt_and_pepper', 'gaussian' 'apply_motion_blur':运动模糊  'add_poisson_noise'
    ...
    for i, batched_input in enumerate(test_pbar):
        ...
        #========================================================lds
        # target_layer = 'model.mask_decoder.output_upscaling'  # 选择要可视化的层
        ...
        with torch.no_grad():
            ...
            # flops = FlopCountAnalysis(model.image_encoder, batched_input["image"])
            # print(f"FLOPs: {flops.total()}")  # 输出 FLOPs 总数            #_----------------------------------------------------------------
        if args.boxes_prompt:
            ...
            save_path = os.path.join(f"{args.work_dir}", args.run_name, f"iter{args.iter_point if args.iter_point > 1 else args.point_num}_danet19_on_best")
        ...
        #print(masks.shape,ori_labels.shape)
