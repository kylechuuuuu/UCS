...
def parse_args():
    ...
    parser.add_argument("--work_dir", type=str, default="weight", help="work dir")
    ...
    parser.add_argument("--sam_checkpoint", type=str, default="./sam_pth/sam_vit_l_0b3195.pth", help="sam checkpoint")
    ...
    parser.add_argument("--save_path", type=str, default="sam_l_1024_halfadap_pca_lds.pth", help="model save")
    parser.add_argument("--save_path_bmt", type=str, default="sam_l_1024_halfadap_pca_ldsbmt", help="model save")
...
def prompt_and_decoder(args, batched_input, model, image_embeddings, decoder_iter = False, interm_embeddings=None ):
#-------------------------------------------------------------------------------------------------------------------------------
...
def train_one_epoch(args, model, optimizer, train_loader, epoch, criterion):
    ...
    for batch, batched_input in enumerate(train_loader):
            ...
            masks, low_res_masks, iou_predictions = prompt_and_decoder(args, batched_input, model, image_embeddings, decoder_iter = False,interm_embeddings=interm_embeddings)
            #-----------------------------------------------------------------------------------------------------------------    
            ...
            for i in range(len(interm_embeddings)):
                interm_embeddings[i] = interm_embeddings[i].detach().clone()
            #-------------------------------------------------------------- 

            init_mask_num = np.random.randint(1, args.iter_point - 1)
            for iter in range(args.iter_point):  #args.iter_point:8
                ...
                masks, low_res_masks, iou_predictions = prompt_and_decoder(args, batched_input, model, image_embeddings, decoder_iter=True,interm_embeddings=interm_embeddings)
                #--------------------------------------------------------------------------------------------------------                 
