...
def get_boxes_from_mask(mask, box_num=1, std = 0.1, max_pixel = 5):
    ...
    #====================================
    if len(boxes) == 0:
        # Get the bounding box of the entire mask
        h, w = mask.shape[:2]
        y0, x0, y1, x1 = 0, 0, h, w
        return torch.tensor([(y0, x0, y1, x1)], dtype=torch.float)
...
def train_transforms(img_size, ori_h, ori_w):
    ...
    # Ensure the image is of type uint8 before transformations
    #transforms.append(A.Lambda(image=lambda x: x.astype('uint8'), mask=lambda x: x.astype('uint8')))
    ...
    transforms.append(A.RandomResizedCrop(height=img_size, width=img_size, scale=(0.8, 1.0), ratio=(0.75, 1.33), p=0.5))
    transforms.append(ToTensorV2(p=1.0))
...
def save_masks(preds, save_path, mask_name, image_size, original_size, pad=None,  boxes=None, points=None, visual_prompt=False):
    ...
    #-----------------------------------------------------------------------------------------------------------------------
    #mask_path = os.path.join(save_path, f"{mask_name.split('.')[0]}.png")
    #-----------------------------------------------------------------------------------------------------------------------
...
#     def __init__(self, weight=20.0, iou_scale=1.0, bce_weight=1.0):
#         super(FocalDiceloss_IoULoss, self).__init__()
#         self.weight = weight
#         self.iou_scale = iou_scale
...
#         self.dice_loss = DiceLoss()
#         self.maskiou_loss = MaskIoULoss()
#         self.bce_loss = nn.BCEWithLogitsLoss()  # 使用 BCE with logits loss

#     def forward(self, pred, mask, pred_iou):
#         """
#         pred: [B, 1, H, W]
#         mask: [B, 1, H, W]
#         pred_iou: [B, 1] (假设这是用于 IoU 计算的预测值)
#         """
#         assert pred.shape == mask.shape, "pred and mask should have the same shape."
...
#         loss1 = self.weight * focal_loss + dice_loss + self.bce_weight * bce_loss
#         loss2 = self.maskiou_loss(pred, mask, pred_iou)
#         loss = loss1 + loss2 * self.iou_scale
#         return loss